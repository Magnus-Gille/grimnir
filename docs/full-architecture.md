...
> Captured at 2026-03-28T20:14:47Z on `huginmunin`
>
...
|------|-------|---------|-----|--------|---------|
| munin-memory.service | ✅ active | enabled | 467278 | n/a | Sat 2026-03-28 15:24:28 CET |
| hugin.service | ✅ active | enabled | 471147 | n/a | Sat 2026-03-28 16:24:51 CET |
| heimdall.service | ✅ active | enabled | 481212 | n/a | Sat 2026-03-28 20:58:15 CET |
| heimdall-collect.service | ❌ inactive
unknown | static | — | — | — |
| heimdall-maintain.service | ❌ inactive
unknown | static | — | — | — |
| ratatoskr.service | ✅ active | enabled | 468603 | n/a | Sat 2026-03-28 15:41:41 CET |
...
|------|--------|-------------|
| munin-memory | main | `0a507b6` chore: add SQLite backup script for NAS (6 hours ago) |
| hugin | main | `015feb4` chore: ignore .claude/ session data directory (4 hours ago) |
| heimdall | main | `3db399f` fix: auto-restart collector on deploy via path watcher (17 minutes ago) |
| skuld | main | `6c28aea` feat: remove Express web server — briefing now lives in Heimdall (34 minutes ago) |
| ratatoskr | main | `8ad5ffa` chore: add session status file (4 hours ago) |
| noxctl | main | `579ac08` docs: update test count and mark completed backlog items (8 days ago) | ⚠️ dirty
| mimir | main | `41d2444` docs: update STATUS.md with script fixes (34 hours ago) |
...
## Phase
Active — memory_history shipped. outcome-aware retrieval Phase 1 also live.

## Current Work
Passive retrieval signal collection live on Pi. memory_insights accumulating with normal use. memory_history exposes audit_log as change feed (29 new tests, 404 total passing).

## Next Steps (prioritized)
1. **Caller context plumbing** — pass auth identity into tool handlers (prerequisite for orient diff)
2. **meta/attention convention** — pilot attention flags via namespace, no schema change
3. **Analyze impact field** across 17 existing debate critique logs — baseline before adding new debate metrics
4. **Outcome-Aware Retrieval Phase 2** — defer until memory_insights shows good signal quality
5. **Migration v5 (optional)** — add entry_id to audit_log rows to link history entries to current state

## Blockers
None.

## Deployed
06d19e9 on Pi (2026-03-27). 404 tests passing. memory_history and memory_insights tools live.
...
## Vision
Task dispatcher that lets any Claude environment submit work to the Pi. The hands of the Grimnir system — "tell it to do X and go to sleep."

## Current Work
Post-task git push safety net active: after successful tasks, Hugin checks for unpushed commits and pushes automatically. Prevents "2 AHEAD" drift visible in Heimdall.
...
- Parallel task execution (currently serial, one-at-a-time)

## Roadmap
- Progress streaming to Telegram/Munin
- Scheduled/recurring tasks (cron-like)
- Task priority levels
- Resource-aware scheduling (memory/CPU checks before spawning)
...
## Vision
At-a-glance health and operational visibility for the entire Grimnir infrastructure. First component with autonomous self-healing capability.

## Current Work
Self-heal module added (`src/self-heal.js`). Deploy task pending on Pi. When a service is unhealthy for 2+ consecutive cycles (~10 min), submits a Hugin task to investigate and restart. Rate-limited 1/service/hour.

## Blockers
None.

## Next Steps
- Verify self-heal deploy succeeded on Pi
- Monitor first autonomous heal event — validate task prompt quality
- Add optimization signal: track heal success rate (did the restart actually fix it?)
- Polish Projects and Deployments tabs
- Consider removing deprecated email code
...
## Vision
Daily intelligence briefing — synthesizes calendar, project state, commitments, and financial data into actionable morning briefings. The oracle of the Grimnir system.

## Current Work
Phase 3 complete: commitment tracking with LLM extraction from briefing context, Munin-backed store, urgency scoring (age + deadline), overdue highlighting, CLI commands. 64 tests passing.

## Blockers
None.

## Next Steps
- Verify systemd timer is running daily at 06:00
- Phase 4: Fortnox financial awareness (invoice aging, revenue pulse, payment status via noxctl)
- Meeting prep cards for upcoming calendar events

## Roadmap
- Phase 4: Fortnox integration
- Phase 5: Meeting prep cards
- Phase 6: Relationship heat maps
- Phase 7: Weekly ritual automation
- Phase 8: Email delivery of briefings
...
## Vision
Telegram router that lets Magnus interact with Grimnir from a phone — send tasks, get results, ask questions.

## Current Work
Fully operational. Two bugs fixed and deployed today: working directory injection for repo tasks, and concierge crash from undefined conversation messages. gille-ai cloned on Pi, grimnir-bot added as collaborator.
...
- Photo/voice message handling (currently silently ignored)
- Task progress notifications
- Rate limiting / debounce on concierge calls

## Ops pattern
Private repos on Pi: `gh api repos/Magnus-Gille/<repo>/collaborators/grimnir-bot -X PUT -f permission=push` on laptop, then accept invite + clone on Pi via `gh repo clone`.
...
## Vision
CLI and MCP server for Fortnox (Swedish accounting platform). Handles invoices, customers, bookkeeping, VAT — making accounting accessible from Claude in any environment.

## Current Work
Published and in maintenance. API drift detection shipped (2026-03-23) — weekly GH Actions workflow diffs Fortnox OpenAPI spec and opens issues on changes. 323 tests passing. npm published as noxctl@0.1.0.

## Blockers
None.

## Next Steps
- Tier 2: confirmation preview, analytics MCP tools, shell completions, natural date periods
- Tier 3: contracts, bank transactions, file attachments
- Release npm 0.2.0

## Roadmap
- 0.2.0: Tier 2 features (confirmation preview, analytics, shell completions)
- 0.3.0: Tier 3 resources (contracts, bank transactions, attachments)
- Claude Desktop auto-registration
- Homebrew formula
...
## Vision
Self-hosted authenticated file server making personal documents available to AI agents over HTTPS. The file archive of the Grimnir system.

## Current Work
Maintenance mode. Share URLs feature shipped — temporary public links via HMAC-signed tokens at /share/:token. Server hardened with standalone rate limiting, trust proxy, cache/referrer headers.

## Recent
- Share URLs: /share skill for Claude Code, Pi-side CLI mints tokens, scripts/share.sh for laptop workflow
- CF Access bypass configured for public share links
- 47 tests passing
...
## Next Steps
- Pull Pi changes from fix-backup-delete task (b71faa1)
- Hugin Phase 2 — email delivery for task results
- 11 open tickets in feedback/munin-memory
...
## Vision
Autonomous collaborator that improves itself and does useful work — both prompted and self-initiated. Four-phase arc: reactive → self-maintaining → proactive → trusted autonomous. Vision doc at `docs/vision.md` (DRAFT v0.1).

## Current Work
Vision document created. Documentation authority map in place (`docs/authority.md`). Hugin port drift fixed. First phase 2 implementation: Heimdall self-heal module (submits Hugin tasks when services are unhealthy). Generator refactor debate completed through Round 2 rebuttal; authority leakage is now the main remaining documentation risk.

## Blockers
None

## Next Steps
1. Verify self-heal deploy on Pi
2. Define doc ownership boundary: `docs/architecture.md` vs `docs/snapshot.md` vs Munin-derived roadmap/status content
3. Refactor `generate-architecture.sh` once that boundary is explicit
4. Run one experimental persona interview, evaluate signal
5. Add optimization signals to other components

## Roadmap
- Phase 2: self-maintaining — expand self-heal to cover more failure modes
- Documentation authority enforcement (generator validates ownership/boundaries, not just conventions)
- Optimization signals per component (Munin relevance, Hugin success rate, Skuld engagement)

---

---

*Snapshot generated at 2026-03-28T20:14:47Z on `huginmunin` by `scripts/generate-architecture.sh`*
