...
| Mimir | Client/accounting files; personal files; generated artifacts | `mimir` | The source folder's class controls. Generated copies must not outlive their source without an explicit reason. | **Current:** Mimir's HTTP surface is read-only—there is no delete API. An operator must remove a file at its authoritative filesystem/source and verify derived copies; encrypted backup history expires through its configured rotation rather than immediate deletion. **Target:** an authenticated, audited deletion workflow that identifies derived copies. |
| Verdandi | Audit and reversal evidence | `verdandi` | Keep evidence for at least as long as the action or retained record it explains. Payload-derived detail should be minimized and may follow the shorter operational window. | **Current:** secrets are redacted before ingest, but immutable original rows cannot be erased by a later tombstone; no tombstone/PII-erasure mechanism is implemented. **Target:** resolve erasure without falsely rewriting hash-chain history. This remains unresolved owning-repo work. |
...
| Backups | Copy of source class | Brokkr | Never extend source retention silently. Rotation may delay physical erasure; record the maximum delay in the owning backup configuration. | **Current:** each backup uses its own configured rotation; there is no cross-store erasure coordinator. **Target:** prove expiry timing and prevent restores from silently resurrecting expired records. |
