...
The broader operational discipline is still:

```text
Execute → Trace → Evaluate → Reflect → Change → Verify
```

That sequence does not collapse the evidence planes or imply automatic mutation. Changes retain
their normal ownership, testing, review, governance, and reversal requirements.
...
| Brokkr | Patch, backup, restore, and host-failure evidence | Substrate evidence; not task quality or model capability. |
...
## Operational trace baseline outside the learning seam

Components still need bounded operational traces for model calls and consequential operations even
when those traces never enter LearningTaskContract. At minimum, record a unique correlation id,
owning component, authenticated principal, stable operation or task type, timing, postcondition
success, and a bounded machine-readable outcome. Optional model, prompt, policy, retry, cost, and
parent-trace fields must be versioned when they drive decisions.

Never retain full prompts, retrieved files, credentials, secrets, or model output merely for easier
debugging. Reference an authorized source record when detail is required, and apply the data
lifecycle to traces, evaluations, and corrections. Shared operational failure labels may include
`wrong_tool`, `bad_args`, `unsupported_claim`, `format_error`, `incomplete`, `timeout`,
`upstream_error`, `policy_denied`, `workspace_error`, `context_stale`, and
`identity_missing`; component-specific extensions must not silently redefine shared labels.

The storage backend is deployment-specific. A durable memory service can hold low-volume summaries
while a local telemetry store buffers high-volume traces. Direct database coupling between
