...
This is a bounded, source-level audit advancing [grimnir#79](https://github.com/Magnus-Gille/grimnir/issues/79). It is not a fleet-wide certification and is **not live-verified**: it does not claim a production write or deployment. Source observations are from the current local owning-repository checkouts on 2026-07-26; live evidence, where required below, remains outstanding.

## Result matrix

| Producer → consumer | Current source evidence | Consumer acceptance / observable result | Evidence status and owner action |
|---|---|---|---|
| Ratatoskr → Heimdall alert ingest | Ratatoskr validates an allowlisted alert and posts the bare object from `src/alert.ts`; Heimdall accepts it at `/api/alerts` in `src/alert-ingest.js`. | A firing alert needs a title; a resolution needs a `dedup_key`. Heimdall normalizes severity and persists title, body/detail, source, and dedup key. It intentionally does not persist `ts` or `links`. | **Gap: consumer-contract test is absent.** Ratatoskr tests prove the outgoing JSON, not Heimdall's actual validator. A Ratatoskr owner ticket must add acceptance fixtures and read-only real-data evidence before this seam is called complete. Ticket routing was not completed in this audit: the GitHub connector refused a cross-repository issue because the target repository's trusted/private status was not verified. |
