...
if (key.asymmetricKeyType !== "ed25519" || !Number.isSafeInteger(Number(sequence)) || Number(sequence) < 1 || (previousDigest && !/^sha256:[a-f0-9]{64}$/.test(previousDigest))) throw new Error("Ed25519 public key, safe positive sequence, and valid previous digest required");
if ((Number(sequence) === 1) !== !previousDigest) throw new Error("sequence 1 requires no predecessor; successors require a previous digest");
const epoch = autonomyEpoch.loadAutonomyContractEpoch(constitutionPath, coveragePath);
const constitution = epoch.constitution, coverage = epoch.coverage;
const attestations = autonomyEpoch.readBoundedJson(attestationsPath, "owner attestation registry");
const recovery = autonomyEpoch.readBoundedJson(recoveryPath, "recovery worker registry");
