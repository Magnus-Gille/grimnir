...
function schemaChecker(rootSchema) {
  ...
  inspect(rootSchema);
  return { valid: (record, label) => assert.deepEqual(errors(rootSchema, record), [], `${label} violates the closed v2 schema`) };
}

const constitutionSchema = read("docs/autonomy-constitution-v2.schema.json");
const journalSchema = read("docs/autonomous-mutation-journal-v2.schema.json");
const coverageSchema = read("docs/autonomy-coverage-registry-v2.schema.json");
...
const constitution = read("docs/autonomy-constitution-v2.json");
const constitutionFixture = read("tests/fixtures/autonomy-contract-v2/constitution.json");
const coverage = read("docs/autonomy-coverage-registry-v2.json");
const armedCoverage = read("tests/fixtures/autonomy-contract-v2/coverage-armed-canary.json");
...
const conformanceText = fs.readFileSync(path.join(root, "docs/autonomy-journal-conformance-v2.md"), "utf8");
const journalPaths = [
  "tests/fixtures/autonomy-contract-v2/journal-happy-commit.json",
  "tests/fixtures/autonomy-contract-v2/journal-r-exact-revert.json",
  "tests/fixtures/autonomy-contract-v2/journal-r-forward-recovery.json",
  "tests/fixtures/autonomy-contract-v2/journal-terminally-blocked.json"
];
...
function constitutionSemantics(record) {
  ...
  for (const entry of record.autonomous_classes) {
    ...
    assert.deepEqual(
      [entry.bounds.apply_verify_budget_seconds, entry.bounds.minimum_watch_seconds, entry.bounds.commit_grace_seconds, entry.bounds.deadline_seconds],
      [300, 3600, 300, 4200],
      `${entry.class} has the owner-approved reachable timing epoch`
    );
...
function journalSemantics(record, c, registry = armedCoverage) {
  ...
  assert.equal(Object.hasOwn(b.canary, "watch_deadline"), false, "watch completion is derived from the durable receipt, never prebound");
  const preparedAt = Date.parse(record.entries[0].recorded_at);
  assert.ok(Date.parse(b.deadline) - preparedAt <= policy.bounds.deadline_seconds * 1000, "attempt deadline stays within constitutional bound");
  ...
  for (const [index, entry] of record.entries.entries()) {
    ...
    if (["prepare", "apply", "verify", "watch", "commit"].includes(entry.phase)) {
      assert.ok(Date.parse(entry.recorded_at) <= Date.parse(b.deadline), "no admission or mutation phase may occur after the bound deadline");
      assert.ok(Date.parse(entry.recorded_at) - preparedAt <= policy.bounds.deadline_seconds * 1000, "every mutation phase stays within the constitutional total deadline");
    }
    if (entry.phase === "watch") {
      assert.ok(Date.parse(entry.recorded_at) - preparedAt <= policy.bounds.apply_verify_budget_seconds * 1000, "prepare through durable watch receipt stays within the apply/readback/verify budget");
      assert.equal(record.entries[index - 1]?.phase, "verify", "the durable watch receipt follows verified candidate readback");
    }
    if (entry.phase === "commit") {
      const watchEntry = record.entries.find((candidate) => candidate.phase === "watch");
      assert.ok(watchEntry, "commit requires a durable authenticated watch receipt");
      const watchDeadline = Date.parse(watchEntry.recorded_at) + policy.bounds.minimum_watch_seconds * 1000;
      assert.ok(Date.parse(entry.recorded_at) >= watchDeadline, "commit waits for the full post-receipt watch window");
      assert.ok(Date.parse(entry.recorded_at) <= watchDeadline + policy.bounds.commit_grace_seconds * 1000, "commit stays within constitutional grace after the earliest valid commit");
    }
...
{
  const happy = journals[0];
  const preparedAt = Date.parse(happy.entries.find((entry) => entry.phase === "prepare").recorded_at);
  const watchAt = Date.parse(happy.entries.find((entry) => entry.phase === "watch").recorded_at);
  const commitAt = Date.parse(happy.entries.find((entry) => entry.phase === "commit").recorded_at);
  assert.equal(watchAt - preparedAt, 300_000, "positive fixture accepts the exact 300-second apply/readback/verify boundary");
  assert.equal(commitAt - watchAt, 3_900_000, "positive fixture accepts the exact 3600-second watch plus 300-second commit-grace boundary");
  assert.equal(Date.parse(happy.binding.deadline) - preparedAt, 4_200_000, "positive fixture accepts the exact 4200-second total-deadline boundary");
}
{
  const inside = clone(journals[0]);
  inside.entries[2].recorded_at = "2026-07-27T00:01:59Z";
  inside.entries[3].recorded_at = "2026-07-27T00:02:00Z";
  inside.entries[4].recorded_at = "2026-07-27T01:02:01Z";
  journalSemantics(resign(inside), constitution, armedCoverage);
}
...
mustReject(() => { const x = resign(clone(journals[0])); x.entries[3].recorded_at = "2026-07-27T00:05:00.001Z"; return resign(x); }, "prepare-to-watch one millisecond beyond 300 seconds is rejected");
mustReject(() => { const x = resign(clone(journals[0])); x.entries.at(-1).recorded_at = "2026-07-27T01:04:59.999Z"; return resign(x); }, "commit one millisecond before the full watch is rejected");
mustReject(() => { const x = resign(clone(journals[0])); x.entries[3].recorded_at = "2026-07-27T00:04:59.999Z"; return resign(x); }, "commit one millisecond beyond grace is rejected independently of the total deadline");
mustReject(() => { const x = resign(clone(journals[0])); x.binding.deadline = "2026-07-27T01:10:00.001Z"; return resign(x); }, "total deadline one millisecond beyond 4200 seconds is rejected");
...
console.log("autonomy-contract v2 validation passed");
