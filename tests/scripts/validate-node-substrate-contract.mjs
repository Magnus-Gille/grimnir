...
function node(record, now) {
  ...
  exact(record.resources, ["cpu_cores", "memory_mib"], "node.resources"); if (![record.resources.cpu_cores, record.resources.memory_mib].every((value) => Number.isInteger(value) && value > 0) || record.capability_status !== "known" || !["arm64", "armv7l", "x86_64"].includes(record.architecture)) fail("node capability is unknown or unsupported");
...
const armv7lNode = structuredClone(positive.records[0]); armv7lNode.node_id = "fixture-pi-zero"; armv7lNode.architecture = "armv7l"; armv7lNode.evidence.evidence_id = "obs-pi-zero-001"; schemaValid(armv7lNode, "armv7l node"); semantic(armv7lNode, Date.parse("2026-07-23T10:15:00Z"));
