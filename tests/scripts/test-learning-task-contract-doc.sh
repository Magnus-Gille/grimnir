...
assert_contains "$CONTRACT" "unsupported schema refs fail" 'external and unresolved local `\$ref` values also fail'
...
assert_contains "$CONTRACT" "multiple quality cohorts fail closed" 'Multiple available.*binding or rubric cohorts make the evaluation ineligible'
assert_contains "$CONTRACT" "v1 has no quality cohort selector" 'independently governed selector'
...
assert_contains "$CONTRACT" "component reviews are recorded" 'Status:.*accepted v1'
assert_contains "$CONTRACT" "Hugin owner approval is linked" 'Hugin owner — approved.*pull/89#issuecomment-5016680034'
assert_contains "$CONTRACT" "gille-inference owner approval is linked" 'gille-inference.*owner — approved.*pull/89#issuecomment-5016680276'
...
assert_contains "$ADR" "ADR is accepted" 'Status:.*accepted'
assert_contains "$ADR" "ADR records Hugin approval" 'Hugin owner review.*approved.*pull/89#issuecomment-5016680034'
assert_contains "$ADR" "ADR records gille-inference approval" 'gille-inference.*owner review.*approved.*pull/89#issuecomment-5016680276'
