.PHONY: docs clean security security-dry deploy test-autonomy-contract test-autonomy-contract-v2 test-autonomy-contract-doc test-autonomy-owner-authorization test-security-skip test-security-delta test-security-completeness test-security-namespace test-security-service-contract test-munin-rpc test-registry-smoke test-placement-validation test-portability-acceptance test-deploy-source-revision test-deploy-persistent-paths test-deploy-systemd-render test-deploy-unit-target-guard test-deploy-boot-enable test-failure-recovery-doc test-cross-service-contract-doc test-fleet-seam-audit-2026-07-26 test-learning-task-contract-doc test-node-substrate-contract test-network-operating-model test-node-substrate-contract-doc test-maintenance-policy-contract test-maintenance-policy-contract-doc test-operational-observability-contract test-operational-observability-contract-doc test-physical-agent-control test-physical-agent-control-doc test-system-roi-ledger test-registry-checkout test-systemd-status test-runtime-state test-worktree-hygiene test-validate-exit test-validation-evidence test-claude-capacity-preflight test-github-project-preflight test-github-actions-policy test-github-actions-zero-step-preflight test-validation-staleness-evidence-doc test-doc-index test-architecture-component-table test-architecture-component-table-regression test-instruction-eval-sandbox test-skills-eval-sandbox test-telemetry-strategy-doc test
...
test-deploy-boot-enable: ## Honor explicit boot-enable policy for system services on clean installs (issue #194)
	@bash scripts/tests/deploy-boot-enable.test.sh
...
test-physical-agent-control-doc: ## Preserve the physical-control authority and safety boundary (issue #179)
	@bash tests/scripts/test-physical-agent-control-doc.sh
